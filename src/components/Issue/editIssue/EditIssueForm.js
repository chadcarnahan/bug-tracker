...
import HandleEditIssue from "./HandleEditIssue";

const EditIssueForm = () => {
  const { showDetails } = useDb();
  const { issueType, priority, summary, component } = showDetails;

  const [description, setDescription] = useState(showDetails.description);
  ...
  return (
    ...
    >
      <div className="flex flex-col space-y-4 w-full mt-1">
        ...
        <HandleEditIssue values={values} description={description} />
      </div>
    </Box>
  );
};
